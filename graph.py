...
class AdjacencySetGraph(Graph):
    ...
    def add_edge(self, v1, v2, weight=1): #only unweighted graphs
        if v1 >= self.numVertices or v2 >= self.numVertices or v1 < 0 or v2 < 0:
            raise ValueError(f"Vertices {v1} and {v2} are out of bounds")

        if weight != 1:
            raise ValueError(f"An adjacency set cannot represent edge weights > 1")
    ...
    def get_adjacent_vertices(self, v):
        if v < 0 or v >= self.numVertices:
            raise ValueError(f"Connot access vertex {v}")
